...
int LLVMFuzzerTestOneInput(const uint8_t *data,size_t size)
{
 ...
 for(i=DSC_PPS_BYTES;i<size;i++){mix^=data[i];sum=(uint8_t)(sum+data[i]);}
 ...
 opt.incr_order=(mix>>7)&1;opt.rc_pipeline=sum&1;opt.scale_dec=(sum>>1)&1;
 opt.partial_target=(sum>>2)&1;opt.very_flat=((sum>>3)&3)%3;opt.partial_padding=(sum>>5)&1;
