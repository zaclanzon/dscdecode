...
int main(int argc,char **argv)
{
 ...
 for(arg=1;arg<argc;arg++){
  ...
  n=fread(seed,1,sizeof(seed),f);fclose(f);
  if(n<128)return 2;
  for(i=0;i<40000;i++){
   unsigned k,changes=1+random32()%16;
   size_t len=n;
   memcpy(work,seed,n);
   if(i%8==0)len=random32()%(n+1);
   else if(i%8==1){for(k=0;k<n;k++)work[k]=(uint8_t)random32();}
   else for(k=0;k<changes;k++){
    size_t base=i%3?128:0;
    if(n<=base)base=0;
    size_t at=base+random32()%(n-base);
    work[at]^=(uint8_t)(1u<<(random32()%8));
   }
   LLVMFuzzerTestOneInput(work,len);total++;
  }
 }
