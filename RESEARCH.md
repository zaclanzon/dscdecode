...
| OQ-24 | Which groups the bitSaveMode update of DSC 1.2b §6.8.4 treats as having flatness signaled for their supergroup. §6.6.3 sends the flag for a supergroup (groups 1–4, 5–8, … of the slice) in the group two before it starts, and the type and position in the next group. Reading A (supergroup): the groups of a supergroup whose flag is 1. Reading B (group): only the flat group the position points at. Reading C (received): the flag received last, so the group that carries a flag and the three after it. Reading D (carrier): the two groups that carry a flag of 1 and the type and position after it. A and B were the first two readings; the model contradicted both (`oq24_bitsave_flat`), and C and D are the readings that fit its output there. | DSC 1.2b §6.8.4, §6.6.3 | `bitsave_flat=received` (default) or `supergroup`, `group`, `carrier` | Pending. `oq24_bitsave_flat` was committed with the A and B predictions (60ae6cc) before the model decoded it; the model's output matched neither. `oq24b_bitsave_flat` and `oq24c_bitsave_flat`, committed with predictions for all four readings before the model decoded them, give each reading a different pair of outcomes. The default is C, judged more likely: it fits `oq24_bitsave_flat` and is what keeping the last flag read would give. |
