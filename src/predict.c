                           ...
                           uint16_t out[4][3], unsigned capacity)
{
    ...
    memcpy(p->history, next, n * sizeof(next[0]));
    p->valid = n;
}
                            ...
                            int ich, const unsigned index[3], uint16_t out[4][3])
{
    ...
    if (!p || !qlevel || !residual || !mpp || !index || !out || x != p->next_x || y != p->next_y ||
    ...
    if (use_bp && !ich && x < (unsigned)-vector) {
        return -1;
    }
    if (!ich) {
        for (c = 0; c < p->f.units; ++c) {
    ...
    if (x + n < p->width) {
        history_update(p, ich, index, out, capacity);
    }
    p->next_x = x + n;
    if (p->next_x == p->width) {
        uint16_t *swap;
